...
from ice import GE_PYTHON_34, IS_PYPY
from ice.parser import Symbol, Keyword, parse, lex, get_temp_name
from .utils import issequence_except_str
from .constants import *
from .exceptions import IceSyntaxError, DuplicatedDefError
...
def add_binding_name(binding_name, file_name, lineno=0):
    return binding_name_set_stack[-1].add_binding_name(binding_name, file_name, lineno)
...
class Translator(object):
    ...
    def translate_block(self, ice_block, filename='<string>',
                        show_tokens=False):
        """Translate sexpressions into a Python AST.
        """
        sexps = parse(lex(ice_block, debug=show_tokens, filename=filename), filename)
        body = []
        for sexp in sexps:
        ...
        return self._check_duplicated_binding(ast.Module(body=body))
        # return ast.Module(body=body)

    def translate_file(self, filename, show_tokens=False):
        """Translate a Ice file into a Python AST.
        """
        self.filename = filename
        with open(filename, 'r') as fobj:
            ice_block = fobj.read()
        return self.translate_block(ice_block, filename=filename,
    ...
    def translate_loaded_file(self, filename, show_tokens=False):
        ...
        with open(filename, 'r') as f:
            sexps = parse(lex(f.read(), debug=show_tokens, filename=filename), filename)
            for sexp in sexps:
                if isinstance(sexp, MutableSequence):
                    sexp = tuple_it(sexp)
                if sexp is COMMENT:
    ...
    def translate_loaded_files(self, file_paths):
        ...
        for file_path in file_paths:
            if not isinstance(file_path, str):
                raise IceSyntaxError(file_path, self.filename)
    ...
    def translate_def(self, exp):
        if len(exp) != 3:
            raise IceSyntaxError(exp, self.filename)
    ...
    def create_assign_targets(self, seq):
        ...
        for index, item in enumerate(seq):
            if isinstance(item, Symbol):
                ...
                if len(item) == 0:
                    raise IceSyntaxError("can't bind to ()", self.filename)
                #if item[0] == 'getattr':
                ...
                raise IceSyntaxError(item, self.filename)
    ...
    class DuplicatedBindingChecker(ast.NodeTransformer):
        ...
        def visit_ClassDef(self, node):
            add_binding_name(node.name, self.filename, node.lineno)
            return self.generic_visit(node)
        ...
        def visit_If(self, node):
            # print('before begin ------')
            # print(node)
            # print(binding_name_set_stack[-1])
            # print('before end --------')
        ...
        def visit_FunctionDef(self, node):
            ...
            for arg in node.args.args:
                add_binding_name(arg if isinstance(arg, str) else arg.arg,
                                 self.filename,
                                 node.lineno)
            if node.args.vararg is not None:
        ...
        def visit_Assign(self, node):
            if not (hasattr(node, 're_assign') and node.re_assign):
                for target in node.targets:
                    add_binding_name(target.id, self.filename, target.lineno)
            return node
    ...
    def _check_duplicated_binding(self, ast_node):
        checker = self.DuplicatedBindingChecker(self.filename)
        return ast.fix_missing_locations(checker.visit(ast_node))
    ...
    def translate_defun(self, exp, decorator_exps=None, visible=True):
        ...
        defn = self._tail_recursion_optimize(defn)
        pre.append(defn)
        ...
        for member_exp in members_exp:
            if issequence_except_str(member_exp):
                if len(member_exp) != 2 and not isinstance(member_exp[0], Symbol):
                    raise IceSyntaxError(member_exp, self.filename)
                member_name_exp, member_type_exp = member_exp
    ...
    def add_macro(self, name, macro_func):
        ...
        if callable(macro_func):
            global_env[name] = macro_func
        else:
            code = compile(
                self._check_duplicated_binding(ast.Module(body=[self.macro_table[name]])),
                # ast.Module(body=[self.macro_table[name]]),
                self.filename, 'exec', optimize=2)
    ...
    def translate_apply(self, exp):
        ...
        for argKey, argExp in chunks(keyword_arg_exps, 2):
            if type(argKey) is not Keyword:
                raise IceSyntaxError(argKey, self.filename)
            arg_pre, arg_value = self.translate(argExp, False)
            pre.extend(arg_pre)
    ...
    @syntax('label')
    def translate_label(self, exp):
        if len(exp) != 3:
            raise IceSyntaxError(exp, self.filename)
        label_symbol, label, value = exp
        if issequence_except_str(value):
            seq = self._translate_sequence(value, False)
            return seq[:-1], seq[-1]
        else:
            return self.translate(value, False)
    ...
    def translate_quasi_quote(self, exp):
        ...
        if issequence_except_str(value):
            if len(value) >= 1 and type(value[0]) is Symbol:
                if value[0].name == 'unquote': # TODO call_funcがついてしまっているので、、
                    ...
                    return (tuple(value1_pre),
                            [self.translate((Symbol('quote'),
                                             Symbol('_ice_unquote_splicing')), False), value1_body])
            lineno = quote_symbol.lineno
            col_offset = quote_symbol.col_offset
            pre = []
            elts = []
    ...
    def translate(self, exp, enclose=True, quoted=False, quasi_quoted=False):
        ...
        if type(exp[0]) is Symbol:
            if exp[0].name == 'use':
                import importlib
                module_name = exp[2].name
                macro_names = exp[1]
                mod = importlib.import_module(module_name)
                for macro_name in macro_names:
                    self.add_macro(macro_name.name, getattr(mod, macro_name.name))
                return self.translate_atom(NONE_SYM)
            if exp[0].name == 'load':
            ...
            elif exp[0].name == 'call_macro':
                if exp[1].name not in self.macro_table:
                    # if is_all_symbol(exp[2]) and exp[3][0] == Symbol('do') and not exp[4]:
                    #     print([exp[1], [Symbol('fn'), exp[2], exp[3]]])
                    #     return self.enclose(self.translate_apply([exp[1], [Symbol('fn'), exp[2], exp[3]]]),
                    #                         enclose)
                    # else:
                    raise IceSyntaxError(exp, self.filename)
                return self.translate(self.expand_macro(exp[1].name, exp[2:]), enclose)
            elif exp[0].name == 'call_func':
                return self.enclose(self.translate_apply(exp[1:]), enclose)
...
class Scope(dict):
    ...
    def merge_children(self):
        ...
        for binding_name in merge_map.keys():
            file_name, lineno = merge_map[binding_name]
            self.add_binding_name(binding_name, file_name, lineno)

    def add_binding_name(self, binding_name, file_name, lineno=0):
        # TODO _gs
        if not binding_name.startswith('_gs') and binding_name in self:
            raise DuplicatedDefError(binding_name, file_name, lineno)
        self[binding_name] = (file_name, lineno)
        return binding_name
